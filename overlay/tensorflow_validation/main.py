...
def create_and_train_classification_model(features_df, label_df) -> tf.keras.Model:
    ...
    model.add(tf.keras.layers.Dense(units=16, activation="relu", name="first_layer"))
    model.add(tf.keras.layers.Dense(units=4, activation="relu", name="second_layer"))
    model.add(tf.keras.layers.Dense(units=1, name="third_layer"))
...
def main():
    ...
    model_type = "classification"  # change this to run either regression or classification
    ...
    if model_type == "classification":
        ...
        features_df, label_df = get_data_for_regression_model()
